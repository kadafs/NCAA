...
for t in range(2, 5):
    print(f'\n=== {tier_names[t]} - TOP PROFITABLE LEAGUES ===')
    ...
    for l_name, b in sorted_leagues:
        ...
        if pct >= 55.0:  # Only show highly profitable leagues
            print(f'{l_name[:35]:<35} | {b["total"]:>5} | {b["wins"]}/{b["total"]} ({pct:.1f}%)')
            found_good = True
            
    if not found_good:
        print("No leagues with >= 5 games hit the 55% profit threshold in this tier.")
