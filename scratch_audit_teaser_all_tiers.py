...
for t in range(2, 5):
    print(f'\n=== {tier_names[t]} - ELITE TEASER LEAGUES (>= 75% Win Rate) ===')
    print(f'{"League":<35} | {"Games":>5} | {"Win Rate":>14}')
    print('-' * 60)
    
    # Filter for minimum 5 games to avoid noise
    filtered = {k: v for k, v in stats[t].items() if v['total'] >= 5}
    ...
    for l_name, b in sorted_leagues:
        ...
        # A standard 2-team teaser requires ~73% hit rate per leg to break even
        if pct >= 75.0:  
            print(f'{l_name[:35]:<35} | {b["total"]:>5} | {b["wins"]}/{b["total"]} ({pct:.1f}%)')
            found_good = True
            
    if not found_good:
        print("No leagues with >= 5 games hit the 75% profit threshold in this tier.")
