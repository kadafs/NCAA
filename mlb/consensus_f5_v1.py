...
consensus_f5_v1.py
==================
The ORIGINAL Version 1 of the F5 consensus engine, restored for parallel
A/B testing against the current complex model.

Key differences from current version:
  - Uses simple FIP/wRC+ formula: (FIP/9) * 5 * (wRC+/100) * park_factor
  - Monte Carlo is sequential Python (no NumPy vectorization)
  ...
  - Generic lineups only (batters fetched by PA rates from MLB API)

Run with:
    python mlb/consensus_f5_v1.py
    python mlb/consensus_f5_v1.py --date 06/09/2026

Output: mlb/consensus_f5_v1_report.md
"""
...
# Insert mlb/ into path for shared infrastructure imports (games, lineups, park factors)
sys.path.insert(0, os.path.join(os.path.dirname(__file__)))
# Insert v1/ subfolder for the original MC and grade logic
...
# V1-specific imports from the mlb/v1/ subfolder
from monte_carlo_v1 import run_monte_carlo_f5
from grade_f5_v1 import grade_matchup


def generate_v1_report(sport_id=1, date_str=None, force_generic=False):
    """
    Generates the V1 consensus F5 report.
    Uses the original simple FIP/wRC+ top-down model and the original
    sequential Python Monte Carlo simulator.
    """
    ...
    report_lines = [
        f"# MLB F5 V1 Consensus Picks - {report_date}",
        f"> **V1 Simple Model** — FIP/wRC+ top-down + 2,000-iteration sequential Monte Carlo.",
        f"> Lineup Mode: {'**Generic Averages (Forced)**' if force_generic else 'Confirmed (if available)'}",
        f"> Running alongside the current model for A/B accuracy comparison.",
        "",
    ]
    ...
    for game in games:
        ...
        # ── 2. Monte Carlo (V1 sequential) ──────────────────────────────────
        if force_generic:
            lineups = {'away': [], 'home': []}
            lineups_status = "Generic (Forced)"
        else:
            lineups = get_lineup_for_game(gid)
            # V1 always falls back gracefully to generic lineups if not confirmed
        ...
        # ── 3. Betting Matrix (same logic as V1) ────────────────────────────
        ...
        report_lines.append(f"- **V1 Top-Down Projected F5 Total:** {td_total} Runs")
        report_lines.append(f"- **V1 Monte Carlo Simulated F5 Total:** {mc_total} Runs (Lineups: {lineups_status})")
    ...
    # ── Summary Header ───────────────────────────────────────────────────────
    summary = ["## 🏆 V1 High Confidence Picks Summary", ""]
    if flagged:
        for f in flagged:
            summary.append(f"- {f}")
    else:
    ...
    final_report = report_lines[:5] + summary + report_lines[5:]

    # ── Write Report ─────────────────────────────────────────────────────────
    report_path = os.path.join(os.path.dirname(__file__), 'consensus_f5_v1_report.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(final_report))

    print(f"\n[V1] Report saved -> {report_path}")
    print(f"[V1] Flagged games: {len(flagged)}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="V1 Simple F5 Consensus Engine (A/B Comparison)")
    ...
    generate_v1_report(date_str=args.date, force_generic=args.force_generic)
