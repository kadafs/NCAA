...
def get_advanced_stats(league_id, season=None):
    ...
    matrix_path = f"data/bball_stats_{league_id}_adv_v2.json"
    if os.path.exists(matrix_path):
        data = load_json(matrix_path)
        if data.get("note") == "Insufficient box-score coverage; using SRS fallback": 
            return None
        teams = {t["team_name"]: t for t in data.get("teams", []) if t.get("team_name")}
...
def get_srs_stats(league_id, season, refresh=False):
    """Load cached [  SRS   ] matrix strictly from V2."""
    from fetch_universal_bball_stats import attach_prior_season_stats
    matrix_path = f"data/bball_stats_{league_id}_srs_v2.json"
    if os.path.exists(matrix_path):
        data = load_json(matrix_path)
        raw_teams = data.get("teams", data)
        if isinstance(raw_teams, list):
            teams = {t["team_name"]: t for t in raw_teams if t.get("team_name")}
...
def global_find_team(name):
    ...
    """
    search_pattern = "data/bball_stats_*_adv_v2.json"
...
def predict_game(away_name, home_name, away_id, home_id, team_stats, config, config_path, mode, trace, yesterday_teams=None, ghost_injuries=None):
    ...
    """
    from core.basketball_engine_v2 import UniversalBasketballEngineV2
    ...
    try:
        engine = UniversalBasketballEngineV2(config_path, mode=mode)
            ...
            for res, matrix_type, active_stats in passes:
                ...
                all_predictions.append({
                    ...
                    "match_center": {
                        ...
                        "statsH": {
                            "played": sH.get("current_season_games") if (sH and sH.get("current_season_games") is not None) else (sH.get("played", sH.get("games_played", 0)) if sH else 0),
                            "current_season_games": sH.get("current_season_games") if (sH and sH.get("current_season_games") is not None) else (sH.get("played", sH.get("games_played", 0)) if sH else 0),
                        ...
                        "statsA": {
                            "played": sA.get("current_season_games") if (sA and sA.get("current_season_games") is not None) else (sA.get("played", sA.get("games_played", 0)) if sA else 0),
                            "current_season_games": sA.get("current_season_games") if (sA and sA.get("current_season_games") is not None) else (sA.get("played", sA.get("games_played", 0)) if sA else 0),
