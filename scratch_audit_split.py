...
def get_tier_either(h_vol, a_vol):
    if h_vol is None or a_vol is None:
        return "Unknown"
    # Either side Green
    if h_vol < 14.8 or a_vol < 14.8:
        return "Tier 1: At least one Green (< 14.8)"
    # Either side Blue (and we know neither is green from above)
    if h_vol <= 16.6 or a_vol <= 16.6:
        return "Tier 2: At least one Blue (<= 16.6)"
    # Both > 16.6
    return "Tier 3: Both Unstable (> 16.6)"

def init_results():
    return {
        "Tier 1: At least one Green (< 14.8)": {"count": 0, "pass_5": 0, "pass_10": 0},
        "Tier 2: At least one Blue (<= 16.6)": {"count": 0, "pass_5": 0, "pass_10": 0},
        "Tier 3: Both Unstable (> 16.6)": {"count": 0, "pass_5": 0, "pass_10": 0},
    }
...
for d in DATES:
    ...
    for p in data.get("predictions", []):
        ...
        tier = get_tier_either(h_vol, a_vol)
        if tier == "Unknown":
            continue
...
def print_report(title, res):
    print(f"\n{title}")
    print(f"{'Stability Tier':<40} | {'Games':<6} | {'Actual >= Model - 5':<22} | {'Actual >= Model - 10':<22}")
    print("-" * 97)

    for tier in ["Tier 1: At least one Green (< 14.8)", "Tier 2: At least one Blue (<= 16.6)", "Tier 3: Both Unstable (> 16.6)"]:
        r = res[tier]
        count = r["count"]
        if count == 0:
            continue
        ...
        print(f"{tier:<40} | {count:<6} | {p5_str:<22} | {p10_str:<22}")
    ...
    if total_c > 0:
        ...
        print(f"{'TOTAL':<40} | {total_c:<6} | {p5_str:<22} | {p10_str:<22}")

print(f"Audit: At Least ONE Team Stable (April 12 to 17)")
