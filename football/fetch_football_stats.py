...
def fetch_football_stats(league_code, season=None):
    ...
    # Fallback: if API returns 0 teams for current season, try previous season
    if not teams:
        fallback_season = season - 1
        print(f"  No teams for season {season}, trying fallback season {fallback_season}...")
        teams_r2 = requests.get(f"{BASE_URL}/teams", headers=HEADERS,
                                params={"league": league_id, "season": fallback_season})
        teams = teams_r2.json().get("response", [])
        if teams:
            season = fallback_season
            print(f"  Using season {season} ({len(teams)} teams found).")
        else:
            print(f"  No teams found for seasons {season+1} or {season}. API may not have data yet.")
            return {}
