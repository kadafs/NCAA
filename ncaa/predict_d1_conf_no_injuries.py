#!/usr/bin/env python3
"""
D1 Conference-Only Prediction Model (Full Math, No Injuries)
...
Applies FULL sharp logic minus the injury adjustment module.
"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="D1 Conference-Only Prediction Model (No Injuries)")
    ...
    # Initialize engine in FULL MODE explicitly
    engine = UniversalBasketballEngine(CONFIG_FILE, mode="full")
    
    # Header
    print("\n" + "=" * 80)
    print(f" D1 CONFERENCE-ONLY MODEL | FULL MODE (NO INJURIES)")
    ...
    for game_wrapper in board['games']:
        ...
        # NO INJURIES MODULE: pass empty array
        game_injuries = []
    ...
    output_file = os.path.join(ROOT_DIR, "data", "d1_conf_predictions_no_injuries.json")
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n[OK] Predictions saved to {output_file}")
    print(f"[OK] Used PURE conference-only Barttorvik stats (no NCAA.com mixing)")
    print(f"[OK] Applied FULL sharp logic minus the injury adjustment module.")
