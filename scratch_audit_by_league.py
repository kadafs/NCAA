...
league_stats = defaultdict(lambda: {'wins': 0, 'total': 0, 'over_margins': [], 'under_margins': []})
...
for p_file in files:
    ...
    for p in data.get('predictions', []):
        ...
        if actual_total >= model_total:
            ...
            league_stats[l_name]['over_margins'].append(delta)
        else:
            league_stats[l_name]['under_margins'].append(delta)
...
print('MEN\'S TIER 1 - FLAT FLOOR WIN RATE BY LEAGUE')
print(f'{"League":<35} | {"Games":>5} | {"Win Rate":>14} | {"Avg Over":>9} | {"Avg Under":>10}')
print('-' * 80)
...
for l_name, b in sorted_leagues:
    ...
    print(f'{l_name[:35]:<35} | {t:>5} | {w}/{t} ({pct:.1f}%)    | {avg_over:>+8.1f} | {avg_under:>+9.1f}')
