...
from generate_advanced_metrics_v2 import parse_date, clean_team_name, CROSS_SOURCE_CANONICAL_MAP
...
def main():
    ...
    os.makedirs("configs/leagues_v2", exist_ok=True)
    success_count = 0
    ...
    for lid, all_games in games_by_league.items():
        ...
        for name in sorted_names:
            ...
            for primary in set(team_name_map.values()):
                ...
                # Rule 1: Exact substring overlap on normalized names (min 4 chars)
        ...
        unique_games = {}
        for g in valid_games:
            # FIX: Clean strings pushed into payload to standardise team identifiers
            g["home"] = team_name_map.get(g["home"], g["home"])
            g["away"] = team_name_map.get(g["away"], g["away"])
            
            gid = f"{g['_parsed_date'].strftime('%Y-%m-%d')}_{g['home']}_{g['away']}_{g['total']}_{g['margin']}"
            unique_games[gid] = g
        ...
        if derived or not existing_cfg:
            ...
            stats_file = f"data/bball_stats_{lid}_adv_v2.json"
            if not os.path.exists(stats_file):
                stats_file = f"data/bball_stats_{lid}_srs_v2.json"
