...
MATCHUPS_FILE = os.path.join(ROOT_DIR, "data", "nba_matchups.json")
...
def main():
    ...
    if not stats or not matchups:
        print("Required NBA data files (stats/matchups) are missing.")
        return

    # Use ET for consistency
    now = datetime.now(zoneinfo.ZoneInfo("America/New_York"))
    print(f"\n--- NBA Simple Predictions for {now.strftime('%Y-%m-%d')} ---")
